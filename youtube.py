...
def api_connection():
    api_key = "AIzaSyDTN8jHK3qFLb8xIICCbBZ9P2-O02sT9tg"
        ...
        if st.button("Run Selected Queries"):
            ...
            for selected_question in selected_questions:
                ...
                if selected_question == questions[0]:
                    ...
                    df['Channel_Num'] = range(1, len(df) + 1)
                    st.write(df)
                    st.subheader('Scatter chart for average duration of videos for each channel')
                    plt.figure(figsize=(10, 6))
                    plt.scatter(df['Channel_Num'], df['Avg_Duration'], color='skyblue')
                    plt.xlabel('Channel')
                    plt.ylabel('Average Duration')
                    plt.title('Average Duration of Videos for Each Channel')
                    plt.xticks(df['Channel_Num'], df['Channel_Name'], rotation=90)
                    plt.tight_layout()
                    st.pyplot()
